...
// Increw Dump Class ID
// {C4E20FFF-37A2-4cec-9ECE-42B9481B4D62}
DEFINE_GUID(CLSID_Dump, 0xc4e20fff, 0x37a2, 0x4cec, 0x9e, 0xce, 0x42, 0xb9, 0x48, 0x1b, 0x4d, 0x62);

// INCREW: This function must be implemented by the user (application)
void DSDumpPrintLog(const char* _msg, ...);
...
//
//
void CDumpInputPin::StrangeCaseFound()
{
    if ( !m_bFoundStrangeCase )
    {
        DSDumpPrintLog("Strange case found!!!!!!!! -> (%s)",StrUtils::uniCharToMultiChar(m_pDump->getFileName()).c_str());
        m_bFoundStrangeCase = true;
    }
}
...
HRESULT CDumpInputPin::WriteStringInfo(IMediaSample *pSample)
{
    ...
    HRESULT hr = StringCchPrintf(FileString,256,TEXT("   Start time (%s)"),(LPCTSTR)CDisp(tStart,CDISP_DEC_FIXED_STR_SIZE));
    m_pDump->WriteString(FileString);

    hr = StringCchPrintf(FileString,256,TEXT("   End time (%s)"),(LPCTSTR)CDisp(tStop,CDISP_DEC_FIXED_STR_SIZE));
    m_pDump->WriteString(FileString);

    // Duration
    REFERENCE_TIME Duration = tStop-tStart;
    if ( Duration != m_tLastDuration )
        hr = StringCchPrintf(FileString,256,TEXT("   Duration (%s)"),(LPCTSTR)CDisp(Duration,CDISP_DEC_FIXED_STR_SIZE));
    else
        hr = StringCchPrintf(FileString,256,TEXT("            (%s)"),(LPCTSTR)CDisp(Duration,CDISP_DEC_FIXED_STR_SIZE));
    m_pDump->WriteString(FileString);
    m_tLastDuration = Duration;

    // Display the media times for this sample
    hr = pSample->GetMediaTime(&tStart, &tStop);
    if (hr == NOERROR) 
    {
        hr = StringCchPrintf(FileString,256,TEXT("   Start media time (%s)"),(LPCTSTR)CDisp(tStart,CDISP_DEC_FIXED_STR_SIZE));
        m_pDump->WriteString(FileString);
        hr = StringCchPrintf(FileString,256,TEXT("   End media time (%s)"),(LPCTSTR)CDisp(tStop,CDISP_DEC_FIXED_STR_SIZE));
        m_pDump->WriteString(FileString);
    }
    ...
    {
        StrangeCaseFound();
        hr = StringCchPrintf(FileString,256,TEXT("   Sync point (%01d)"),(hr == S_OK));
    }
    else
        hr = StringCchPrintf(FileString,256,TEXT("              (%01d)"),(hr == S_OK));
    m_pDump->WriteString(FileString);

    // Is this a preroll sample
    hr = pSample->IsPreroll();
    if ( hr == S_OK )
        hr = StringCchPrintf(FileString,256,TEXT("   Preroll (%01d)"),(hr == S_OK));
    else
        hr = StringCchPrintf(FileString,256,TEXT("           (%01d)"),(hr == S_OK));
    m_pDump->WriteString(FileString);
    ...
    {
        StrangeCaseFound();
        hr = StringCchPrintf(FileString,256,TEXT("   Discontinuity (%d)"),(hr == S_OK));
    }
    else
        hr = StringCchPrintf(FileString,256,TEXT("                 (%d)"),(hr == S_OK));
    m_pDump->WriteString(FileString);
    ...
    {
        StrangeCaseFound();
        hr = StringCchPrintf(FileString,256,TEXT("   Type changed (%01d)\0"), (pMediaType ? TRUE : FALSE));
    }
    else
        hr = StringCchPrintf(FileString,256,TEXT("                (%01d)\0"), (pMediaType ? TRUE : FALSE));
    m_pDump->WriteString(FileString);
    DeleteMediaType(pMediaType);

    // Write the actual data length
    LONG DataLength = pSample->GetActualDataLength();
    if ( DataLength != m_LastDataLength )
        hr = StringCchPrintf(FileString,256,TEXT("   Actual data length (%d)"),DataLength);
    else
        hr = StringCchPrintf(FileString,256,TEXT("                      (%d)"),DataLength);
    m_LastDataLength = DataLength;
    m_pDump->WriteString(FileString);
...
{
    m_tLast = 0;
    m_pDump->WriteString("!!!!!!!!!!!!!!!!!!!!! NEW SEGMENT !!!!!!!!!!!!!!!!!!!!!!!!!!!\r\n\0");
    return S_OK;
